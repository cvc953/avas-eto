...
void main() {
  test('colored quadrant contains font tag and hex color', () async {
    ...
    final tarea = await buildTarea('t1', DateTime.now().add(const Duration(days: 1)), prioridad: 'Alta');
  ...
  test('overdue text produces human readable strings', () async {
    final ns = NotificationService();
    final tareaMinutes = await buildTarea('m1', DateTime.now().subtract(const Duration(minutes: 5)));
    final t1 = ns.getOverdueText(tareaMinutes);
    expect(t1.contains('Venció hace'), isTrue);

    final tareaHours = await buildTarea('h1', DateTime.now().subtract(const Duration(hours: 3)));
    final t2 = ns.getOverdueText(tareaHours);
    expect(t2.contains('hora') || t2.contains('horas'), isTrue);

    final tareaDays = await buildTarea('d1', DateTime.now().subtract(const Duration(days: 2)));
